...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (locationToggled && locationFilter) {
      currentFilteredPosts = currentFilteredPosts.filter((post) =>
        post.location && typeof post.location === 'string' && post.location.toLowerCase().includes(locationFilter.toLowerCase())
      );
    }

    if (categoryToggled && categoryFilter) {
      currentFilteredPosts = currentFilteredPosts.filter((post) =>
        post.category && typeof post.category === 'string' && post.category.toLowerCase().includes(categoryFilter.toLowerCase())
      );
    }

    setFilteredPosts(currentFilteredPosts);
  }, [posts, searchQuery, locationFilter, categoryFilter, locationToggled, categoryToggled]);
    ...
    <div className="flex h-screen flex-col bg-base">
      ...
      <div className="flex flex-grow flex-row overflow-hidden">
        <div className="flex w-1/6 min-w-[8rem] flex-col overflow-y-auto bg-base px-3 pt-3 shadow-xl shadow-crust">
          ...
          <div className="mt-2 grid h-auto w-full grid-cols-1 md:grid-cols-2 justify-center gap-2 rounded-lg bg-mantle p-3 shadow-sm shadow-crust">
            <div className="flex justify-center">
              <button
                className={`w-full h-auto whitespace-normal rounded-lg border-2 border-lavender px-2 py-1 text-xs md:px-3 md:py-1.5 md:text-sm font-semibold transition-all duration-200 ease-in-out hover:scale-105 active:scale-95 ${locationToggled ? "bg-lavender text-mantle hover:bg-opacity-90" : "text-lavender hover:bg-lavender/10"}`}
            ...
            <div className="flex justify-center">
              <button
                className={`w-full h-auto whitespace-normal rounded-lg border-2 border-lavender px-2 py-1 text-xs md:px-3 md:py-1.5 md:text-sm font-semibold transition-all duration-200 ease-in-out hover:scale-105 active:scale-95 ${categoryToggled ? "bg-lavender text-mantle hover:bg-opacity-90" : "text-lavender hover:bg-lavender/10"}`}
          ...
          <div className="flex h-[50px] flex-shrink-0 items-center justify-between bg-base px-4">
            ...
            {isEmployer && (
              ...
              >
                <span>New Post</span>
                <FaCirclePlus
                  className="cursor-pointer text-2xl text-text transition-transform duration-200 ease-in-out group-hover:text-lavender group-hover:scale-110 group-hover:rotate-12"
                />
              </button>
            )}
          </div>
             ...
             <div className="grid grid-cols-1 gap-4 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3">
              {filteredPosts.length > 0 ? (
                 filteredPosts.map((job) => (
                   <div key={job.postId} className="transition-transform duration-200 hover:scale-[1.02]">
                     <PostCard
                       postId={job.postId}
                       title={job.title}
      ...
      <div
        className={`fixed inset-0 z-40 flex items-center justify-center bg-crust/80 transition-opacity duration-300 ease-out ${
          showNewPostPopout ? "opacity-100" : "opacity-0 pointer-events-none"
        }`}
      >
        <div
          onClick={(e) => e.stopPropagation()}
          className={`relative mx-5 w-full max-w-3xl rounded-lg bg-base p-6 shadow-xl transition-all duration-300 ease-out ${
            showNewPostPopout ? "opacity-100 scale-100" : "opacity-0 scale-95"
          }`}
        >
          {showNewPostPopout && (
             <NewPostPopout
               onClose={() => setShowNewPostPopout(false)}
             />
          )}
        </div>
      </div>
...
const NewPostPopout = ({ onClose }) => {
  ...
  return (
    <>
      <button
        onClick={onClose}
        className="absolute right-4 top-4 text-3xl text-lavender hover:text-opacity-80 transition-colors"
      >
        &times;
      </button>
      ...
      <form className="flex flex-col space-y-4" onSubmit={handleSubmit}>
        <div>
          ...
          <input
            ...
            required
            className="h-10 w-full rounded-lg bg-mantle px-3 text-subtext0 placeholder-surface2 focus:border-2 focus:border-lavender focus:outline-none transition-colors"
          />
        </div>
        <div>
        ...
        <div>
          ...
          <textarea
            ...
            required
            rows={6}
            className="w-full rounded-lg bg-mantle px-3 py-2 text-subtext0 placeholder-surface2 focus:border-2 focus:border-lavender focus:outline-none transition-colors"
          />
        </div>
        <button
          type="submit"
